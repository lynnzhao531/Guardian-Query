...
def generate_candidates(
    ...
    not_terms = get_full_method_not(target_method, progress)
    prev_hashes = set()
    if previous_queries:
    ...
    # Extra discovered terms to mix in
    extra_m_terms = (discovered_terms or {}).get(target_method, [])
    ...
    while len(candidates) < n:
        ...
        # Mix in discovered terms occasionally
        if extra_m_terms and random.random() < 0.3:
