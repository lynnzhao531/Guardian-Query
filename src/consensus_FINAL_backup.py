...
MODEL_THRESHOLDS: Dict[str, Dict[str, float]] = {
    # Thresholds calibrated on outputs/test_100_continuous.csv for Layer-2
    # continuous-output models, and test_100_results.csv for the LLM models.
    # Chosen as the lowest threshold keeping LOW FP <= 10%.
    "m1":    {"decision": 0.25, "method": 0.25},   # Sonnet discrete: 72% rec @ 5% FP
    "m2old": {"decision": 0.20, "method": 0.20},   # Feb SFT discrete: 69% @ 0% FP
    "m2new": {"decision": 0.20, "method": 0.20},
    "m3":    {"decision": 0.70, "method": 0.70},   # Continuous M3: 45% @ 0% FP
    "m4":    {"decision": 0.40, "method": 0.40},   # Continuous M4-v3: 31% @ 8% FP
    "m5":    {"decision": 0.25, "method": 0.25},   # Continuous M5: 97% @ 2% FP
    "m6":    {"decision": 0.25, "method": 0.25},   # Haiku: TBD after testing
}
# Rollback copy — matches old universal score>=1.0 (equivalent to p1>=0.80)
