...
class GuardianClient:
    ...
    def fetch_pages(self, query: str, pages: int | None = None,
                    page_size: int = 50,
        ...
        v3c (Fix 1): adaptive page count + random page offset.
          - supply > 1000 → up to 10 pages (500 articles)
          - supply > 500  → up to 8 pages (400 articles)
          - supply ≤ 500  → up to 5 pages (250 articles, legacy)
        A random start_page is chosen within [1, max_pages - pages + 1]
        so repeated calls to the same query see different slices.
        Attaches ``_fetch_range`` on the returned list as a tuple
        (start_page, end_page, max_pages) for manifest logging.
        """
        import math
        import random as _rnd

        if total_available <= 0:
            total_available = self.preflight(query, section=section)
        max_pages = max(1, math.ceil(total_available / page_size)) if total_available > 0 else 1

        # Adaptive page count (honour explicit override if provided)
        if pages is None:
        ...
        # Random offset to avoid always fetching the same top slice
        if max_pages > pages:
            start_page = _rnd.randint(1, max_pages - pages + 1)
        else:
            start_page = 1
        ...
        # Attach fetch-range metadata for caller (manifest logging)
        try:
            articles._fetch_range = (start_page, end_page, max_pages)  # type: ignore[attr-defined]
        except Exception:
            pass
        self._last_fetch_range = (start_page, end_page, max_pages)
        logger.info("fetch_pages: query hits=%d, pages %d-%d of %d (got %d articles)",
                    total_available, start_page, end_page, max_pages, len(articles))
        return articles
