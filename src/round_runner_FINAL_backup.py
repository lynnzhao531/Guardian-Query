...
def run_round(round_id: int, *, dry_run: bool = False) -> dict:
    ...
    try:
        ...
        # Forced method rotation for the first 12 rounds after the baseline
        # is first captured. Baseline is persisted so it survives process
        # restarts; each new round advances the offset by exactly 1.
        # 2 rounds per method, covers all 6. Bandit takes over from offset 12+.
        _METHOD_CYCLE = ["rct", "case_study", "prepost",
