...
def run_round(round_id: int, *, dry_run: bool = False) -> dict:
    ...
    try:
        ...
        try:
            ...
            _tier_a_min = 3
        try:
            import vocabulary_discovery as _vd_init
            _disco_snapshot = _vd_init.load_state()
            _disco_counts = {
                "strong_total": sum(len(v) for v in _disco_snapshot.get("strong", {}).values()),
                "trial_total": sum(len(v) for v in _disco_snapshot.get("trial", {}).values()),
                "last_run_round": int(_disco_snapshot.get("last_run_round", 0) or 0),
            }
        except Exception:
            _disco_counts = {"strong_total": 0, "trial_total": 0, "last_run_round": 0}
        manifest["config_snapshot"] = {
            ...
            "discovered_terms_count": _disco_counts,
            "bandit_rho": 0.92,
            "feature_dim": 12,
        }
        ...
        if 0 <= _idx < 6:
            ...
            # STUCK DETECTOR v3b (Fix 3): if the current target had scored < 10
            # in 3 of its last 5 targeted attempts, switch to the least-explored
            # method from the recent window. Protects against SEEN_URLS saturation.
            # Also: 20% exploration floor (Fix 2) — if any method has been untried
            # for ≥10 rounds, with p=0.2 force-pick it instead of the bandit choice.
            ...
            try:
                ...
                for _p in _recent[-20:]:
                    try:
                        ...
                        pass
                # Fix 3: 3 of last 5 rounds targeting this method had scored < 10
                _target_recent = [
                    _m for _m in _recent_manifests if _m.get("target_method") == target
                ][-5:]
                _low_count = sum(
                    1 for _m in _target_recent if int(_m.get("scored_count", 0) or 0) < 10
                )
                if len(_target_recent) >= 3 and _low_count >= 3:
                    ...
                    if _least != target:
                        logger.warning(
                            "STUCK DETECTOR: %s had %d/%d recent rounds with scored<10; "
                            "switching to least-explored method %s",
                            target, _low_count, len(_target_recent), _least,
                        )
                ...
                # Fix 2: 20% exploration floor for methods untried in last 10 rounds
                _last10 = _recent_manifests[-10:]
                _recent_targets = {_m.get("target_method") for _m in _last10}
                _untried = [m for m in METHODS if m not in _recent_targets]
                if _untried and _rnd.random() < 0.20:
                    _pick = _rnd.choice(_untried)
                    if _pick != target:
                        logger.warning(
                            "EXPLORATION FLOOR: %s untried in last 10 rounds; "
                            "forcing target %s → %s", _untried, target, _pick,
                        )
                        manifest["exploration_floor_fired"] = True
                        manifest["exploration_floor_from"] = target
                        manifest["exploration_floor_to"] = _pick
                        target = _pick
        ...
        # Fix 2: per-method saturation = 1 - avg unique_rate of last 3 rounds targeting m
        method_saturation: dict = {}
        try:
            _recent_all = sorted(
                glob.glob(str(ROUNDS_DIR / "round_*" / "round_manifest.json")),
                key=lambda p: int(os.path.basename(os.path.dirname(p)).split("_")[1]),
            ...
            _rm = []
            for _p in _recent_all[-30:]:
                try:
                    _rm.append(json.load(open(_p)))
                except Exception:
                    pass
            for m in METHODS:
                _last3 = [x for x in _rm if x.get("target_method") == m][-3:]
                if not _last3:
                    method_saturation[m] = 0.0
                    continue
                _rates = [float(x.get("unique_rate", 0) or 0) for x in _last3]
                _avg = sum(_rates) / len(_rates)
                method_saturation[m] = max(0.0, min(1.0, 1.0 - _avg))
        except Exception as _e:
            logger.warning("saturation calc failed (non-fatal): %s", _e)
            method_saturation = {m: 0.0 for m in METHODS}

        # Fix 1: run vocabulary discovery every 5 rounds (before loading terms)
        try:
            import vocabulary_discovery as _vd
            _disco_state = _vd.load_state()
            if int(round_id) % 5 == 0 and int(_disco_state.get("last_run_round", 0)) != int(round_id):
        ...
        # Fix 1: load discovered + trial vocabulary (post-discovery)
        _strong_terms: dict = {m: [] for m in METHODS}
        _trial_terms: dict = {m: [] for m in METHODS}
        try:
            import vocabulary_discovery as _vd
            _strong_terms = _vd.get_strong_terms()
            _trial_terms = _vd.get_trial_terms()
        except Exception as _e:
            logger.warning("vocabulary_discovery load failed (non-fatal): %s", _e)

        cands = generate_candidates(
            target, n=30,
            method_progress=method_progress,
            round_num=round_id,
            method_saturation=method_saturation,
            discovered_terms=_strong_terms,
            trial_terms=_trial_terms,
        )
        ...
        manifest["query_type"] = chosen.get("query_type", "standard")
        ...
        raw = guardian.fetch_pages(chosen["query"], page_size=50,
                                   total_available=total,
        ...
        # v3c Fix 1C: log pagination range for forensic analysis
        if hasattr(guardian, "_last_fetch_range"):
            _sp, _ep, _mp = guardian._last_fetch_range
            manifest["pages_fetched"] = f"{_sp}-{_ep}"
            manifest["max_pages"] = int(_mp)
        ...
        # v3c Fix 5: skip scoring when too few unique articles survive dedup.
        # Saves budget + API calls on rounds doomed to yield nothing.
        if len(unique_articles) < 5:
            logger.warning("Skipping scoring: only %d unique articles after dedup (<5)",
                           len(unique_articles))
            manifest["skip_reason"] = "insufficient_unique"
            scored_set = []
        elif chosen.get("is_decision_only"):
            # v3c Fix 3: decision-only rounds score at most 30 articles, and
            # prioritise titles containing *any* method term (from any of the
            # 6 methods) so downstream models have something to classify.
            from query_builder import METHOD_TERMS as _MT
            _all_method_terms = []
            for _mlist in _MT.values():
                _all_method_terms.extend(t.lower() for t in _mlist)
            def _priority_key(a):
                tl = (a.get("title") or "").lower()
                return 0 if any(mt in tl for mt in _all_method_terms) else 1
            _ordered = sorted(unique_articles, key=_priority_key)
            scored_set = _ordered[:30]
            manifest["decision_only_scored"] = len(scored_set)
            logger.info("decision_only round: scoring %d articles (method-term priority)",
                        len(scored_set))
        else:
        ...
        # Fix 6: Trial-term graduation / attrition (only if trial terms were used)
        try:
            _trial_used = list(chosen.get("trial_terms_used") or [])
            if _trial_used:
                import vocabulary_discovery as _vd2
                _grad = _vd2.update_trial_results(
                    round_id=int(round_id),
                    target_method=target,
                    terms_used=_trial_used,
                    tier_a_gained=len(tier_a_hits),
                )
                manifest["trial_graduation"] = _grad
        except Exception as _e:
            logger.warning("trial graduation failed (non-fatal): %s", _e)
            ...
            for tm, cv in credits.items():
                ...
                if url not in _pool_urls(pp):
                    _append_csv(pp, [{
                        ...
                        "confidence": c.get("confidence", 0),
                        "method_certainty": c.get("method_certainty", ""),
                        "credit": cv, "round_id": int(round_id), "source": "consensus",
                        "article_relevance_score": c.get("article_relevance_score", 0),
                        "models_agreeing_high": c.get("models_agreeing_high", 0),
                    }], POOL_A_HDR)
