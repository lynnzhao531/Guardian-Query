...
"v1 (rounds 1-10): 4-5 AND clauses, 4 models, single threshold.
 ...
 v2 (rounds 11-33): 2 AND + 1 NOT, 5 models, two-tier, method rotation, 6-component reward.
 v3 (rounds 34+): 6 active models (M5 DistilBERT + M6 Haiku added),
   per-model thresholds recalibrated (decision ≥0.25 floor blocks score=0.5
   articles; method thresholds disabled for M1/M3/M4/M6),
   Tier A raised to ≥3/6 genuine majority, unconditional STATIC NOT clause,
   bandit V-floor 0.05, BANDIT_STATE reset."

#### Per-Model Threshold Table (v3):
| Model  | Decision | Method | Notes |
|--------|----------|--------|-------|
| M1     | 0.25     | 0.05   | Sonnet, method disabled (near-random) |
| M2-old | 0.25     | 0.25   | Conservative rejector, both enabled |
| M2-new | 0.25     | 0.25   | Under monitoring, both enabled |
| M3     | 0.70     | 0.05   | Continuous output, method disabled |
| M4     | 0.50     | 0.05   | Ridge continuous, method weak |
| M5     | 0.30     | 0.30   | DistilBERT, dec==meth single vote |
| M6     | 0.25     | 0.05   | Haiku wrapped, method disabled |
...
"1. Read CLAUDE.md to understand the project structure.
 ...
 3. The pipeline searches The Guardian for articles matching a specific
    structural pattern (method + decision). It uses 6 independent AI models
    (M1 Sonnet, M2-old/M2-new GPT SFT, M3 MLP, M4 Ridge, M5 DistilBERT, M6 Haiku)
    to score articles, requires ≥3/6 model consensus for high-confidence
