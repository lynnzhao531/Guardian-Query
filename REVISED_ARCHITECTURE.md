...
# §2. THE 6 ACTIVE MODELS (v3 configuration)

| ID | Engine | K*? | Status | Notes |
|---|---|---|---|---|
| M1 | Claude Sonnet (temp=0) | Yes | Active | Decision-driven (method threshold disabled = 0.05) |
| M2-old | ft:gpt-4.1-mini (Feb) | No | Active | Conservative rejector; both decision+method thresholds at 0.25 |
| M2-new | New fine-tuned gpt-4.1-mini | Yes | Under monitoring | Both thresholds at 0.25 (see §2.1) |
| M3 | sentence-transformers + MLP | K*-FREE | Active | Continuous output, decision-driven (method disabled) |
| M4 | v3 continuous K* features + Ridge | Yes | Active | Best method classifier (11/15); method threshold disabled |
| M5 | DistilBERT 3-class (local) | No | Active | Zero API cost; single relevance bit broadcast to dec==meth |
| M6 | Haiku + 3 K* + sub-questions | Yes | Active | Independent cheap LLM (~$0.08/100 articles); method disabled |
...
Entry: in Tier B + ≥3 of 6 models say HIGH (agree on decision HIGH).
Starting threshold: 3/6. Genuine majority of 6 active models.
Rationale: ≥2 would effectively mean "M1 says yes" (M5/M6 often align with M1).
Lower bound: 3. Do NOT drop below 3 without evidence from 30+ rounds.
...
## §4.4 NOT Clause — THREE components (STATIC + FULL-METHOD)

Component 0 (v3): Components 1 and 2 form the STATIC NOT clause — applied
UNCONDITIONALLY to every query (see `query_builder.get_static_not()`). Never
omitted regardless of broaden/narrow preflight adjustments.
...
## v3 (2026-04-04) — FINAL PRE-FLIGHT activation
- M5 (DistilBERT) and M6 (Haiku + K*) added to active scoring loop → 6 models.
- Per-model discrete thresholds recalibrated (consensus.py MODEL_THRESHOLDS):
  decision 0.25 floor ensures score=0.5 articles (p1=0.20) never enter Tier A.
  Method thresholds disabled (0.05) for M1/M3/M4/M6 — distributions didn't
  discriminate HIGH from LOW.
- Tier A threshold raised to 3/6 (FIX 2) — genuine majority across 6 models.
- Unconditional STATIC NOT clause (FIX 3): GLOBAL_EXCLUDE + METHOD_SPECIFIC_EXCLUDE
  applied to every query regardless of preflight adjustments.
- Bandit reward V-floor 0.05 (FIX 9) so exploration isn't killed when
  unique_scored is briefly zero.
- BANDIT_STATE reset (FIX 10): 33 prior rounds ran against 5-model config with
  old thresholds; the posterior was no longer representative.

## v2 (prior) — see docs/versions/ for earlier snapshots.
